...
static TEXTURE_PATH: &str = concat!(env!("CARGO_MANIFEST_DIR"),
    "/src/textures/rust-logo-512x512-blk.png");
...
fn init_instance() -> VdResult<Instance> {
    ...
    Instance::builder()
        ...
        .enabled_extensions(loader.enumerate_instance_extension_properties()?.as_ref())
...
fn create_swapchain(surface: SurfaceKhr, device: Device, window_size: Option<Extent2d>,
    ...
    let surface_format = choose_swap_surface_format(swapchain_details.formats.as_ref());
    let present_mode = choose_swap_present_mode(swapchain_details.present_modes.as_ref());
